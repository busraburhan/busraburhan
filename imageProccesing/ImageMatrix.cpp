...
ImageMatrix ImageMatrix::operator+(const ImageMatrix &other) const {
    ImageMatrix matrixSum(height,width);
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            matrixSum.data[i][j] = data[i][j] + other.data[i][j];
        }
    }
    return matrixSum;
...
ImageMatrix ImageMatrix::operator-(const ImageMatrix &other) const {
    ImageMatrix matrixDifference(height,width);
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            matrixDifference.data[i][j] = data[i][j] - other.data[i][j];
        }
    }
...
ImageMatrix ImageMatrix::operator*(const double &scalar) const {
    ImageMatrix matrixMultiply(height,width);
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            matrixMultiply.data[i][j] = data[i][j] *scalar;
        }
    }
